...
from dataloader import get_data_for_test
...
test_loader = get_data_for_test(params)

class VAE(nn.Module):
    def __init__(self):
        super(VAE, self).__init__()

        self.fc1 = nn.Linear(4096 * 3, 1000)
        self.fc21 = nn.Linear(1000, 100)
        self.fc22 = nn.Linear(1000, 100)
        self.fc3 = nn.Linear(100, 1000)
        self.fc4 = nn.Linear(1000, 4096 * 3)
    ...
    def forward(self, x):
        # print('x', x.size())
        mu, logvar = self.encode(x.view(-1, 4096 * 3))
    ...
    with torch.no_grad():
        for i, (data, _) in enumerate(train_loader):
            ...
            if i == 0:
                n = min(data.size(0), 8)
                # print(data.size())
                comparison = torch.cat([data.view(args.batch_size, 3, 64, 64)[:n],
